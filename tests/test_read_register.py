...
def test_reads_int16_with_the_itools_unit_id(clients, settings):
    assert read_register(clients, settings, "programmer", "HeaterUtil") == 40
    # device_id=255 is mandatory; without it the nanodac connects but never replies
    assert clients["pb1"].reads == [(43874, 1, UNIT_ID)]
    assert UNIT_ID == 255
...
def test_decodes_negative_int16(settings):
    clients = {"pb1": FakeClient({43874: (-12, DATATYPE.INT16)})}
    assert read_register(clients, settings, "programmer", "HeaterUtil") == -12


def test_ignores_trailing_metadata_on_the_register_entry(clients, settings):
    settings["addresses"]["modbus"]["programmer"]["HeaterUtil"] = ["pb1", "43874", 10, "%"]
    assert read_register(clients, settings, "programmer", "HeaterUtil") == 40
