...
- Results are saved to one CSV per program run (`resolve_save_path` is called once at startup, not per test), and `save_csv` appends a row per completed test, writing the header only if the file is new. A row's columns are `Timestamp`, `Elapsed (min)` (test duration in minutes, 2 decimal places, timed with `time.monotonic()` from the start of `test_cracking_pressure` to when the stop condition fires), `Temperature`, then one column per pressure channel.
